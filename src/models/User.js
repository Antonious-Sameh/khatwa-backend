...
const userSchema = new mongoose.Schema(
  {
    ...
    // ── DEPRECATED ── kept only so existing single-device bindings are not
    // lost; auth.controller migrates this into `devices` on next login and
    // no longer writes to this field. Safe to ignore/remove in the future
    // once all students have logged in at least once post-migration.
    deviceId: {
      type:    String,
      default: null,
      select:  false,
    ...
    // Multi-device binding (students only, max 2 — enforced in auth.controller).
    // Not enforced for teachers, who may log in from any number of devices.
    // Each entry represents one bound device/browser.
    devices: {
      type: [
        {
          id:         { type: String, required: true }, // per-browser id generated by the frontend (lib/deviceId.js)
          label:      { type: String, default: null },   // best-effort human-readable device/browser info, for the teacher UI
